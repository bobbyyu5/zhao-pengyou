 ...
 */
export default function Card({ card, size, selected, legal, glow, illegal, friendCall, level, trumpSuit, onClick, back, backId, style }) {
  ...
  if (back) {
    ...
    return <div className={`card back cb-${id} ${isSeal ? "seal-glyph" : ""} ${size || ""}`} style={style} aria-hidden="true" />;
  }
  ...
  if (card.suit === "JOKER") {
    cls.push("is-trump");
    return (
      <button type="button" className={cls.join(" ")} style={style} onClick={onClick}
  ...
  return (
    <Tag type={onClick ? "button" : undefined} className={cls.join(" ")} style={style} onClick={onClick}
      aria-label={`${SUIT_SYMBOL[card.suit]}${rankLabel(card.rank)}`}>
      <span className={`index ${colorClass}`}>
        {rankLabel(card.rank)}
        <span className="mini">{SUIT_SYMBOL[card.suit]}</span>
      </span>
